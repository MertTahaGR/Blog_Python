...
class RegisterForm(Form):
    ...
    password = StringField("Şifre Giriniz:", validators=[
...
def update(id):
    if request.method == "GET":
        ...
        if result == 0:
            flash("Böyle Bir Makale Yok/BU işleme Yetkiniz yok","danger")
            return redirect(url_for("index"))
        else:
            article = cursor.fetchone()
            form = ArticleForm()
        ...
        mysql.connection.commit()
        flash("Makale Düzenlendi","success")
        return redirect(url_for("dashboard"))
...
def delete(id):
    ...
    if result > 0:
        ...
        flash("Böyle Bir Makale Bulunamadı/Bu İşleme yetkiniz yok","danger")
        return redirect(url_for("index"))
